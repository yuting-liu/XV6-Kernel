...
// Return the proc existed in the system
int
getnumproc(void) {
	struct proc *p;
	int count = 0;
...
// sets the priority for calling process
int setpri(int num, struct proc *p) {
  p -> priority = num;
...
int getpinfo(struct pstat *cur_pro) {
  ...
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
    ...
    cur_pro->pid[i] = p->pid;
    cur_pro->hticks[i] = p->high_tick;
    cur_pro->lticks[i] = p->low_tick;
    ++i;
  }
  release(&ptable.lock);
  return 0;
